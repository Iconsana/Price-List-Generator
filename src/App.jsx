...
const hideElements = () => {
// Hide the main container except preview
const mainContainer = document.querySelector(’.container’);
const allElements = mainContainer?.querySelectorAll(’.bg-white.rounded-xl.shadow-xl’);

```
  allElements?.forEach((el, index) => {
    if (index === 0) { // First element is the form container
      el.style.display = 'none';
    }
  });
  
  // Hide auto-save indicator
  const autoSave = document.querySelector('.fixed.bottom-4.right-4');
...
const showElements = () => {
  ...
  const autoSave = document.querySelector('.fixed.bottom-4.right-4');
  if (autoSave) autoSave.style.display = '';
};
      ...
      <style dangerouslySetInnerHTML={{
        __html: `
          @media print {
            ...
            /* Remove shadows and borders from printed content */
            .shadow-xl {
            ...
            * {
            ...
            /* Hide any buttons that might be in the preview */
            button {
              display: none !important;
            }
          }
